  ...
  return (
    ...
    <ChevronLeftIcon
          ...
          />
            ...
            {
                electroniccat.map((card) => (
                    <Link key={card.id} href={'/product/'+card.id}>
                    <Image className="snap-start bg-origin-padding w-28 h-28 tablet:w-48 tablet:h-48 border-none cursor-pointer hover:scale-90 ease-in duration-150" src={card.image} alt={card.id} width={100} height={100} key={card.id} ></Image>
                    </Link>
